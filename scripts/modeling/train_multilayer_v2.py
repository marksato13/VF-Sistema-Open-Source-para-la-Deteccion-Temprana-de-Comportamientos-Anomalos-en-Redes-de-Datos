...
def episode_mean(rows, names):
    grouped={}
    for r in rows: grouped.setdefault(r['episode_id'],[]).append(r)
    out=[]
    for eid,items in grouped.items():
        out.append({'episode_id':eid,**{k:str(sum(float(x[k]) for x in items)/len(items)) for k in names}})
    return out
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--normal',type=Path,required=True); ap.add_argument('--anomalies',type=Path,required=True); ap.add_argument('--output',type=Path,required=True); ap.add_argument('--aggregate-episodes',action='store_true'); a=ap.parse_args()
    ...
    names,_=features(train)
    if a.aggregate_episodes: train, val, test, anomalies = [episode_mean(x,names) for x in (train,val,test,anomalies)]
