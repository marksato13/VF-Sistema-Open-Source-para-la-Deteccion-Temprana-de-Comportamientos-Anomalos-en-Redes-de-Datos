...
La construcción definitiva omite `--audit-only` y solo ocurre si existen exactamente las 135 celdas válidas, no hay campañas inválidas y el repositorio está limpio:

```bash
python3 scripts/dataset/build_f1_dataset.py
```

El destino por defecto es `artifacts/datasets/f1-normal-v1/`, fuera de Git. Si ya existe, el script se detiene; nunca reemplaza un dataset silenciosamente.
