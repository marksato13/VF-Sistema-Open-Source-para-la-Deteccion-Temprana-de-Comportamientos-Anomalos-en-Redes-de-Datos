...
def main():
 ...
 report={'schema_version':'multilayer-v2-audit','normal_rows':len(n),'anomaly_rows':len(z),'normal_episodes':len({r['episode_id'] for r in n}),'anomaly_episodes':len({r['episode_id'] for r in z}),'feature_count':len(FEATURES),'layer_feature_counts':dict(LAYERS),'missing_by_feature':missing,'constant_features':constants,'duplicate_feature_vectors_total':dup_vectors,'episode_split_violations':split_violations,'normal_labels':dict(normal_labels),'anomaly_labels':dict(anomaly_labels),'normal_sha256':hashlib.sha256(a.normal.read_bytes()).hexdigest(),'anomaly_sha256':hashlib.sha256(a.anomalies.read_bytes()).hexdigest(),'gates':{'schema_complete':not missing_schema,'no_missing_values':not any(missing.values()),'no_episode_split':not split_violations,'normal_labels_clean':set(normal_labels)<=({'normal'}),'anomaly_labels_clean':set(anomaly_labels)<=({'anomaly'})}}
