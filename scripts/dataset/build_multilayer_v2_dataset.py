...
def main():
 p=argparse.ArgumentParser(); p.add_argument("--features-root",type=Path,required=True); p.add_argument("--output",type=Path,required=True); p.add_argument("--partition-map",type=Path,required=True); a=p.parse_args()
 ...
 for cid,part in mapping.items():
  ...
  for row in csv.DictReader(path.open()):
   ...
   row["episode_id"]=cid; row["partition"]=part; rows.append(row); episode_parts.setdefault(cid,set()).add(part)
 ...
 a.output.parent.mkdir(parents=True,exist_ok=True); fields=("episode_id","partition",*META,*FEATURES)
