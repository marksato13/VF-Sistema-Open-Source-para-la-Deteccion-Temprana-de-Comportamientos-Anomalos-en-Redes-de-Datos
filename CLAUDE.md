...
- El preflight de `HTTP-C8/R01` se detuvo sin crear artefactos porque el Sensor perdió la marca `NTPSynchronized=yes` después de unas 18 horas sin alcanzar sus fuentes públicas por la NIC externa aislada. Se diseñó la jerarquía VM01 `10.10.10.10`→Sensor→VM03–VM05. En la prueba runtime, `prefer` solo quedó esperando fuentes NTS requeridas por `authselectmode mix`; añadir `require` —sin `trust`— seleccionó VM01 y recuperó `NTPSynchronized=yes`, estrato 4 y `Leap status: Normal`. C8 permanece bloqueado hasta desplegar esa opción de forma persistente y pasar offsets máximos de 100 ms con `scripts/f1/check_ntp_gate.sh`. Diseño: `docs/05-plan-pruebas/16-correccion-ntp-interno-G7.md`.
