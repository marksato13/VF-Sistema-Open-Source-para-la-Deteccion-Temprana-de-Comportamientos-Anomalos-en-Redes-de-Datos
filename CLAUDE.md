...
- El preflight de `HTTP-C8/R01` se detuvo sin crear artefactos porque el Sensor perdió la marca `NTPSynchronized=yes` después de unas 18 horas sin alcanzar sus fuentes públicas por la NIC externa aislada. C8 permanece bloqueado hasta aplicar la jerarquía VM01 `10.10.10.10`→Sensor→VM03–VM05, exigir fuente seleccionada y offsets máximos de 100 ms con `scripts/f1/check_ntp_gate.sh`. Diseño: `docs/05-plan-pruebas/16-correccion-ntp-interno-G7.md`.
